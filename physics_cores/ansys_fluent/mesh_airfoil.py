...
The workflow is:
    ...
    2. Load a .dxf geometry file containing the airfoil + far-field domain.
...
def generate_airfoil_mesh(
    ...
    Args:
        dxf_file:      Path to a .dxf geometry file generated by the Geometry Agent.
        output_mesh:   Output path for the exported .msh.h5 mesh file.
        length_unit:   Unit of the geometry file dimensions (default is "m").

    Returns:
        Absolute path to the generated .msh.h5 file.
    """
    dxf_file = os.path.abspath(dxf_file)
    print(f"Using provided DXF geometry: {dxf_file}")

    # ── Step 1: Launch Fluent in meshing mode ──────────────────────────────
    ...
    # because our DXF is already perfectly clean from the Geometry Agent.
    ...
    # selection type tells the mesher to name boundaries based on the DXF
    # layer names (e.g. "airfoil", "inlet", "outlet").
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dxf",
        required=True,
        help="Path to .dxf geometry file generated by the Geometry Agent.",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--unit",
        default="m",
        help="Length unit in the DXF file (default: m).",
    )
