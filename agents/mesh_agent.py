...
Output is a native gmsh MSH (v2.2 ASCII) with physical groups for each
boundary zone. `gmshToFoam` (OpenFOAM) reads this directly — no format
conversion needed.
"""
...
class MeshAgent:
    ...
    def generate_mesh(self, step_path: str, name: str = "airfoil") -> str:
        """
        Generates the mesh via gmsh subprocess. Returns absolute path to
        the gmsh MSH file (input to `gmshToFoam`).
        """
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if "MESH_SUCCESS" in output and result.returncode == 0:
                    self.logger.info(f"  gmsh succeeded: {gmsh_path}")
                    return gmsh_path
                else:
